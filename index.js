...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            ...
            + 'FN: OWNER GANS\n' // NAMA KONTAK OWNER
            + 'ORG:Creator ICHI;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=6282338122626:+62-816-5466-368\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '!'
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6282338122626@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"6282338122626@s.whatsapp.net"]
			const adminbotnumber = ["6282338122626@s.whatsapp.net"]
			const frendsowner = ["6282338122626@s.whatsapp.net"]
			const premium = ["6282338122626@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
					client.sendMessage(from, help(prefix), text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6285770269605-1614125991@g.us" } : {}) }, message: { conversation: `MENU MHYW BOT VERSI 2` }}})
					break
					case 'listmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isGroup) return reply(mess.only.group)
